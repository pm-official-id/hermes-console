...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const current = navItems.find((item) => item.href === location)?.label ?? (location.startsWith('/services/') ? 'Service detail' : 'Hermes Console');

  return (
    <div className="app-shell">
      <aside className={cn('sidebar', mobileOpen && 'sidebar-open')}>
        <div className="sidebar-brand">
          <div className="brand-mark"><span>H</span><i /></div>
          <div><div className="brand-name">Hermes</div><div className="brand-caption">local control plane</div></div>
        ...
        <div className="sidebar-bottom">
          <div className="adapter-card">
            ...
            <div className="adapter-name">Hermes local socket</div>
      ...
      <main className="main-shell">
        <header className="topbar">
          <div className="topbar-left"><button className="icon-button mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation" data-testid="button-open-navigation"><Menu size={20} /></button><div className="topbar-context"><span className="eyebrow">Hermes / {current}</span><span className="topbar-path">machine view</span></div></div>
...
function ServicesPage() {
  ...
  return (
    <div className="content-wrap surface-grid">
      <PageHeader eyebrow="Service catalog" title="The managed fleet." description="Start, stop, inspect, and tune every service from one deliberate surface." actions={<button className="button button-primary" onClick={() => void services.refetch()} data-testid="button-scan-services"><RefreshCw size={15} /> Scan host</button>} />
      <div className="catalog-toolbar fade-up fade-up-delay-1"><label className="search-field"><Search size={16} /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search services, images, categories" aria-label="Search services" data-testid="input-search-services" />{search && <button className="clear-search" onClick={() => setSearch('')} aria-label="Clear search" data-testid="button-clear-search"><X size={14} /></button>}</label><div className="filter-group" role="group" aria-label="Service filters">{['all', 'running', 'stopped', 'llm', 'coding', 'automation'].map((item) => <button key={item} className={cn('filter-button', filter === item && 'filter-active')} onClick={() => setFilter(item)} data-testid={`button-filter-${item}`}>{item === 'all' ? 'All services' : categoryLabels[item] ?? item}</button>)}</div></div>
      <QueryState loading={services.isLoading} error={services.error} onRetry={() => void services.refetch()}>{filtered.length ? <div className="catalog-list">{filtered.map((service) => <ServiceCard service={service} key={service.id} />)}</div> : <div className="empty-state large-empty" data-testid="empty-services"><Search size={24} /><h3>{search ? 'No matching services' : 'The catalog is clear'}</h3><p>{search ? 'Try a service name, category, or status.' : 'Hermes will show managed containers here after a host scan.'}</p>{search && <button className="button button-quiet" onClick={() => setSearch('')} data-testid="button-reset-search">Reset search</button>}</div>}</QueryState>
    </div>
  );
}
...
function ConfigForm({ service }: { service: Service }) {
  ...
  return (
    <form className="config-form" onSubmit={save} data-testid="form-service-configuration">
      ...
      <label className="toggle-field"><input type="checkbox" checked={configured} onChange={(event) => setConfigured(event.target.checked)} data-testid="input-service-configured" /><span className="toggle-visual"><i /></span><span><strong>Configuration complete</strong><small>Allow Hermes to include this service in safe actions.</small></span></label>
...
function ServiceDetail({ service }: { service: Service }) {
  const accentStyle = { '--service-accent': service.accent || '#1e928c' } as CSSProperties;
  return <div className="detail-layout" style={accentStyle}><div className="detail-main"><div className="detail-heading fade-up"><div className="detail-icon"><Box size={22} /></div><div><div className="eyebrow accent-eyebrow">{categoryLabels[service.category]}</div><h1 className="page-title">{service.name}</h1><p className="page-description">{service.description}</p></div><div className="detail-status"><StatusDot status={service.status} label={service.status} /><ServiceActionButtons service={service} /></div></div><div className="detail-stats fade-up fade-up-delay-1"><div><span>Health</span><StatusDot status={service.health} /></div><div><span>Uptime</span><strong>{service.uptime || '—'}</strong></div><div><span>CPU</span><strong>{service.cpuPercent}%</strong></div><div><span>Memory</span><strong>{service.memory}</strong></div><div><span>Connections</span><strong>{service.connections}</strong></div></div><LogsPanel serviceId={service.id} /></div><aside className="detail-side"><section className="panel config-panel fade-up fade-up-delay-1"><div className="panel-heading"><div><span className="eyebrow">Service configuration</span><h2>Runtime</h2></div><FileCog size={18} className="panel-icon" /></div><ConfigForm service={service} /></section><section className="panel image-panel fade-up fade-up-delay-2"><span className="eyebrow">Container source</span><code>{service.image}</code><div className="image-meta"><span><TimerReset size={14} /> {service.uptime || 'not running'}</span><span><Link2 size={14} /> :{service.port}</span></div></section></aside></div>;
}
...
function ConnectionsPage() {
  ...
  return (
    <div className="content-wrap surface-grid">
      ...
      <QueryState loading={connections.isLoading} error={connections.error} onRetry={() => void connections.refetch()}>{connections.data && (connections.data.length ? <div className="connections-layout"><section className="panel connection-map-panel"><div className="panel-heading"><div><span className="eyebrow">Observed paths</span><h2>{connections.data.length} active route{connections.data.length === 1 ? '' : 's'}</h2></div><span className="map-legend"><span className="legend-line" /> data path</span></div><div className="connection-list">{connections.data.map((connection) => <button className={cn('connection-row', selectedConnection?.id === connection.id && 'connection-row-active')} onClick={() => setSelected(connection.id)} key={connection.id} data-testid={`button-connection-${connection.id}`}><div className="connection-endpoint"><span className="endpoint-badge">{connection.source.slice(0, 1)}</span><strong>{connection.source}</strong></div><div className="connection-route"><span>{connection.protocol}</span><div className="route-rule"><i /></div><ArrowRight size={15} /></div><div className="connection-endpoint target-endpoint"><span className={cn('endpoint-badge target', statusTone(connection.status))}>{connection.target.slice(0, 1)}</span><strong>{connection.target}</strong></div><StatusDot status={connection.status} /></button>)}</div></section><aside className="panel connection-inspector"><span className="eyebrow">Route inspector</span>{selectedConnection ? <><div className="inspector-route"><span>{selectedConnection.source}</span><ArrowRight size={18} /><span>{selectedConnection.target}</span></div><StatusDot status={selectedConnection.status} label={selectedConnection.status} /><div className="inspector-details"><div><span>Protocol</span><strong className="font-mono-ui">{selectedConnection.protocol}</strong></div><div><span>Detail</span><strong>{selectedConnection.detail}</strong></div><div><span>Connection ID</span><strong className="font-mono-ui">{selectedConnection.id}</strong></div></div><button className="button button-quiet full-width" onClick={() => window.alert('Connection diagnostics are available through the host adapter.')} data-testid="button-test-connection"><Zap size={14} /> Run diagnostic</button></> : <div className="empty-state compact-empty"><Link2 size={20} /><strong>Select a route</strong><span>Choose a path to inspect its protocol and current state.</span></div>}</aside></div> : <div className="empty-state large-empty"><Network size={25} /><h3>No connections yet</h3><p>When Hermes detects a compatible runner or interface, its path will appear here.</p><Link href="/services" className="button button-primary" data-testid="link-browse-services">Browse services <ArrowRight size={14} /></Link></div>)}</QueryState>
    </div>
  );
}
...
function SettingsPage() {
  ...
  const [adapter, setAdapter] = useState(() => localStorage.getItem('hermes-adapter') ?? 'Hermes local socket');
